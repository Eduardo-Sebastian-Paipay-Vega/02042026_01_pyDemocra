...
const ALLOWED_LOGIN_ROLE_IDS = [ROLE_IDS.admin, ROLE_IDS.principal, ROLE_IDS.trabajador, ROLE_IDS.voluntario] as const;
...
const parseIdListFromCsv = (raw: unknown): number[] => {
  ...
  // Support Postgres array literal "{1,2}" or comma separated "1,2".
...
const resolveOrganizationIdsFromUser = (user?: Partial<User> | null): number[] => {
  if (!user) return [];

  const fromArray = Array.isArray((user as any).organizationIds)
    ? (user as any).organizationIds
    : null;

  const ids = fromArray ? parseIdListFromCsv(fromArray) : [];
  if (ids.length > 0) return Array.from(new Set(ids));

  // organizationId can be a single id or an accidental serialization of an array.
  const fromPrimary = parseIdListFromCsv((user as any).organizationId);
...
const mapDbUserToSystemUser = (usuario: any): User => {
  ...
  const orgIds = parseIdListFromCsv(usuario?.id_organizacion);
  const organizationIds = orgIds.length > 0 ? orgIds.map(String) : undefined;
  const organizationId = orgIds.length > 0 ? String(orgIds[0]) : undefined;
...
const sanitizeUserForResponse = (user: User) => ({
  ...
  organizationIds: (user as any).organizationIds || undefined,
  organizationName: user.organizationName,
  organizations: (user as any).organizations || undefined,
...
const enrichSessionUserOrganizations = async (user: User): Promise<User> => {
  const orgIds = resolveOrganizationIdsFromUser(user);
  if (orgIds.length === 0) return user;
  ...
  if (orgError) {
    console.warn('No se pudo resolver organizaciones para sesion:', orgError.message);
    return user;
  }

  const organizations = (orgRows || [])
    .map((row: any) => ({
      id_organizacion: Number(row.id_organizacion),
      nombre: String(row.nombre || '').trim(),
    }))
    .filter((row: any) => Number.isInteger(row.id_organizacion) && row.id_organizacion > 0 && row.nombre);

  const primaryOrg = organizations.find((row: any) => String(row.id_organizacion) === String(user.organizationId))
    || organizations[0]
    || null;

  return {
    ...user,
    organizationId: user.organizationId || (primaryOrg ? String(primaryOrg.id_organizacion) : undefined),
    organizationIds: orgIds.map(String),
    organizationName: user.organizationName || (primaryOrg ? primaryOrg.nombre : null),
    organizations,
  } as any;
};
...
app.post('/auth/login', async (c) => {
  try {
    ...
    if (!ALLOWED_LOGIN_ROLE_IDS.includes(Number(usuarios.id_rol) as typeof ALLOWED_LOGIN_ROLE_IDS[number])) {
      ...
      return c.json({ error: 'Acceso denegado. Solo admin, principal, trabajador y voluntario pueden iniciar sesion.' }, 403);
    }
    ...
    const enrichedUser = await enrichSessionUserOrganizations(user);

    // Guardar en KV para mantener compatibilidad con el resto del sistema
    const kvKey = `users:${enrichedUser.id}`;
    await kv.set(kvKey, enrichedUser);

    // Generar JWT personalizado
    const { generateJWT } = await import('./auth.ts');
    const accessToken = await generateJWT(enrichedUser);

    return c.json({
      access_token: accessToken,
      user: sanitizeUserForResponse(enrichedUser),
    });
  } catch (error: any) {
...
app.get('/volunteers', authMiddleware, async (c) => {
  try {
    ...
    if (!['admin', 'principal', 'trabajador'].includes(actorRole)) {
      return c.json({ error: 'Sin permisos para listar voluntarios' }, 403);
    }

    const actorOrgIds = resolveOrganizationIdsFromUser(actor);
    const estadoIndex = await refreshEstadosCache();
    let query = supabase
      .from('usuarios')
      .select(`
    ...
    if (actorRole === 'trabajador') {
      if (actorOrgIds.length === 0) return c.json({ error: 'Usuario sin organizacion asignada' }, 403);
      query = query.in('id_organizacion', actorOrgIds);
    }

    const { data: volunteers, error } = await query.order('id_usuario', { ascending: false });
...
app.get('/volunteers/:id', authMiddleware, async (c) => {
  try {
    ...
    const volunteerIdInt = Number(volunteerId);
    if (!Number.isInteger(volunteerIdInt) || volunteerIdInt <= 0) {
      return c.json({ error: 'ID de voluntario invalido' }, 400);
    }

    const actor = c.get('user') as User;
    const actorRole = getSessionRoleName(actor);
    const actorIdInt = await resolveUserIdInt(actor, supabase);
    const actorOrgIds = resolveOrganizationIdsFromUser(actor);

    if (!['admin', 'principal', 'trabajador', 'voluntario'].includes(actorRole)) {
      return c.json({ error: 'Sin permisos para ver voluntarios' }, 403);
    }

    if (actorRole === 'voluntario' && actorIdInt !== volunteerIdInt) {
      return c.json({ error: 'Sin permisos para ver este voluntario' }, 403);
    }
    ...
    const volunteerOrgIds = parseIdListFromCsv((volunteer as any).id_organizacion);

    if (actorRole === 'trabajador') {
      if (actorOrgIds.length === 0) {
        return c.json({ error: 'Usuario sin organizacion asignada' }, 403);
      }
      if (volunteerOrgIds.length === 0 || !volunteerOrgIds.some((id) => actorOrgIds.includes(id))) {
        return c.json({ error: 'Sin permisos para ver voluntarios de otra organizacion' }, 403);
      }
    }

    const allowedOrgIds = (actorRole === 'trabajador' || actorRole === 'voluntario')
      ? (actorOrgIds.length > 0 ? actorOrgIds : volunteerOrgIds)
      : null;

    if ((actorRole === 'trabajador' || actorRole === 'voluntario') && (!allowedOrgIds || allowedOrgIds.length === 0)) {
      return c.json({ error: 'Usuario sin organizacion asignada' }, 403);
    }
    ...
    const activityIds = [...new Set((relaciones || [])
      .map((r: any) => Number(r?.id_actividad))
      .filter((id: number) => Number.isInteger(id) && id > 0))];

    const activityOrgMap = new Map<number, number | null>();
    const responsibleNameMap = new Map<string, string>();
    const responsibleOrgMap = new Map<string, number | null>();
    const orgNameMap = new Map<number, string>();

    if (activityIds.length > 0) {
      const { data: activityOrgRows, error: activityOrgError } = await supabase
        .from('actividades')
        .select('id_actividad, id_responsable')
        .in('id_actividad', activityIds);

      if (activityOrgError) {
        throw new Error(`Error al obtener actividades del voluntario: ${activityOrgError.message}`);
      }

      const responsibleIds = [...new Set((activityOrgRows || [])
        .map((row: any) => Number(row?.id_responsable))
        .filter((id: number) => Number.isInteger(id) && id > 0))];

      if (responsibleIds.length > 0) {
        const { data: responsables, error: respError } = await supabase
          .from('usuarios')
          .select('id_usuario, nombre_completo, id_organizacion')
        ...
        for (const resp of responsables || []) {
          responsibleNameMap.set(String(resp.id_usuario), resp.nombre_completo || 'Sin responsable');
          const orgIds = parseIdListFromCsv((resp as any).id_organizacion);
          responsibleOrgMap.set(String(resp.id_usuario), orgIds.length > 0 ? orgIds[0] : null);
        }
      }

      const orgIdsToLookup = Array.from(new Set(
        [...Array.from(responsibleOrgMap.values()), ...volunteerOrgIds]
          .filter((id) => Number.isInteger(id) && Number(id) > 0)
          .map((id) => Number(id)),
      ));

      if (orgIdsToLookup.length > 0) {
        const { data: orgRows, error: orgError } = await supabase
          .from('organizaciones')
          .select('id_organizacion, nombre')
          .in('id_organizacion', orgIdsToLookup);

        if (orgError) {
          console.warn('No se pudo resolver nombre de organizaciones:', orgError.message);
        } else {
          for (const row of orgRows || []) {
            const id = Number((row as any).id_organizacion);
            const nombre = String((row as any).nombre || '').trim();
            if (Number.isInteger(id) && id > 0 && nombre) orgNameMap.set(id, nombre);
          }
        }
      }

      const fallbackOrgId = volunteerOrgIds.length > 0 ? volunteerOrgIds[0] : null;
      for (const row of activityOrgRows || []) {
        const idActividad = Number((row as any).id_actividad);
        if (!Number.isInteger(idActividad) || idActividad <= 0) continue;
        const respId = (row as any).id_responsable ? String((row as any).id_responsable) : '';
        const respOrg = respId ? (responsibleOrgMap.get(respId) ?? null) : null;
        activityOrgMap.set(idActividad, respOrg ?? fallbackOrgId);
      }
    }

    const orgRestricted = Array.isArray(allowedOrgIds) && allowedOrgIds.length > 0;
    const allowedActivityIds = orgRestricted
      ? activityIds.filter((id) => {
          const orgId = activityOrgMap.get(id) ?? (volunteerOrgIds.length > 0 ? volunteerOrgIds[0] : null);
          return orgId !== null && allowedOrgIds!.includes(orgId);
        })
      : activityIds;

    const allowedActivityIdSet = new Set<number>(allowedActivityIds);
    const stats = (relaciones || []).reduce((acc: any, rel: any) => {
      const idActividad = Number(rel?.id_actividad);
      if (!Number.isInteger(idActividad) || idActividad <= 0) return acc;
      if (!allowedActivityIdSet.has(idActividad)) return acc;
      acc.hours += Number(rel?.horas_total || 0);
      acc.activities.add(String(idActividad));
      return acc;
    ...
    if (allowedActivityIds.length > 0) {
      const { data: dbActivities, error: activitiesError } = await supabase
        ...
        .in('id_actividad', allowedActivityIds)
      ...
      activities = (dbActivities || []).map((activity: any) => {
        const idActividad = Number(activity.id_actividad);
        const orgId = Number.isInteger(idActividad)
          ? (activityOrgMap.get(idActividad) ?? (volunteerOrgIds.length > 0 ? volunteerOrgIds[0] : null))
          : null;
        const orgName = orgId ? (orgNameMap.get(orgId) || null) : null;
        ...
        return {
          ...
          responsibleName: responsibleNameMap.get(String(activity.id_responsable)) || 'Sin responsable',
    ...
    const estadoNombreVolunteer = resolveEstadoName(null, volunteer.id_estado, 'general', estadoIndex);
    const mappedVolunteer = {
      ...
      organizacion: (volunteer as any).organizacion || null,
      id_organizacion: volunteerOrgIds.length > 0 ? volunteerOrgIds[0] : null,
      areaId: volunteer.id_area ? String(volunteer.id_area) : '',
      availability: normalizeAvailability(volunteer.disponibilidad),
      status: mapDbVolunteerStatusToLegacy(estadoNombreVolunteer),
      id_estado: Number(volunteer.id_estado) || null,
      estado: estadoNombreVolunteer,
...
app.get('/voluntarios/:id/historial-horas', authMiddleware, async (c) => {
  try {
    ...
    if (!['admin', 'principal', 'trabajador', 'voluntario'].includes(actorRole)) {
      return c.json({ success: false, error: 'Sin permisos para ver historial' }, 403);
    }

    if (actorRole === 'voluntario' && actorIdInt !== volunteerId) {
      return c.json({ success: false, error: 'Sin permisos para ver este historial' }, 403);
    }

    const { data: volunteerRow, error: volunteerError } = await supabase
      .from('usuarios')
      .select('id_usuario, id_rol, id_organizacion')
      .eq('id_usuario', volunteerId)
      .eq('id_rol', ROLE_IDS.voluntario)
      .maybeSingle();

    if (volunteerError) {
      throw new Error(`Error al validar voluntario: ${volunteerError.message}`);
    }

    if (!volunteerRow) {
      return c.json({ success: false, error: 'Voluntario no encontrado' }, 404);
    }

    const volunteerOrgIds = parseIdListFromCsv((volunteerRow as any).id_organizacion);
    if (actorRole === 'trabajador') {
      if (actorOrgIds.length === 0) return c.json({ success: false, error: 'Usuario sin organizacion asignada' }, 403);
      if (volunteerOrgIds.length === 0 || !volunteerOrgIds.some((id) => actorOrgIds.includes(id))) {
        return c.json({ success: false, error: 'Sin permisos para ver voluntarios de otra organizacion' }, 403);
      }
    }
    ...
    if ((actorRole === 'trabajador' || actorRole === 'voluntario') && (!allowedOrgIds || allowedOrgIds.length === 0)) {
      return c.json({ success: false, error: 'Usuario sin organizacion asignada' }, 403);
    }
    ...
    const itemsRaw = (data || []).map((row: any) => ({
      ...
      actividad: row.actividades
        ? {
            ...
            id_responsable: row.actividades.id_responsable ?? null,
            id_creador: row.actividades.id_creador ?? null,
    ...
    const allActivityIds = [...new Set((totalRows || [])
      .map((row: any) => Number(row?.id_actividad))
      .filter((id: number) => Number.isInteger(id) && id > 0))];

    const activityOrgMap = new Map<number, number | null>();
    const userOrgMap = new Map<string, number | null>();
    const orgNameMap = new Map<number, string>();

    if (allActivityIds.length > 0) {
      const { data: activityRows, error: activityErr } = await supabase
        .from('actividades')
        .select('id_actividad, id_responsable, id_creador')
        .in('id_actividad', allActivityIds);

      if (activityErr) {
        throw new Error(`Error al obtener actividades para filtro de organizacion: ${activityErr.message}`);
      }

      const userIds = Array.from(new Set((activityRows || [])
        .flatMap((row: any) => [Number(row?.id_responsable), Number(row?.id_creador)])
        .filter((id: number) => Number.isInteger(id) && id > 0)));
      ...
      if (userIds.length > 0) {
        ...
        if (usersErr) {
          throw new Error(`Error al obtener organizacion de usuarios: ${usersErr.message}`);
        }

        for (const u of userRows || []) {
          const orgIds = parseIdListFromCsv((u as any).id_organizacion);
          userOrgMap.set(String((u as any).id_usuario), orgIds.length > 0 ? orgIds[0] : null);
        }
      }

      const orgIdsToLookup = Array.from(new Set(
        [...Array.from(userOrgMap.values()), ...volunteerOrgIds]
          .filter((id) => Number.isInteger(id) && Number(id) > 0)
          .map((id) => Number(id)),
      ));
      ...
      if (orgIdsToLookup.length > 0) {
        ...
        if (orgErr) {
          console.warn('No se pudo resolver nombre de organizaciones en historial KoBo:', orgErr.message);
        } else {
          for (const row of orgRows || []) {
            const id = Number((row as any).id_organizacion);
            const nombre = String((row as any).nombre || '').trim();
      ...
      for (const row of activityRows || []) {
        ...
        if (!Number.isInteger(idActividad) || idActividad <= 0) continue;
        const respId = (row as any).id_responsable ? String((row as any).id_responsable) : '';
        const creadorId = (row as any).id_creador ? String((row as any).id_creador) : '';
        const respOrg = respId ? (userOrgMap.get(respId) ?? null) : null;
        const creadorOrg = creadorId ? (userOrgMap.get(creadorId) ?? null) : null;
        activityOrgMap.set(idActividad, respOrg ?? creadorOrg ?? fallbackOrgId);
      }
    }
    ...
    const items = (itemsRaw || []).map((item: any) => {
      ...
      if (!orgRestricted) return true;
      const orgId = Number(item?.id_organizacion);
      return Number.isInteger(orgId) && allowedOrgIds!.includes(orgId);
    });
    ...
    const horas_total_bd = totalError
      ? null
      : (totalRows || []).reduce((sum: number, row: any) => {
          const horas = Number(row?.horas_total) || 0;
          if (!orgRestricted) return sum + horas;
          const idActividad = Number(row?.id_actividad);
          if (!Number.isInteger(idActividad) || idActividad <= 0) return sum;
          const orgId = activityOrgMap.get(idActividad) ?? (volunteerOrgIds.length > 0 ? volunteerOrgIds[0] : null);
          if (!orgId || !allowedOrgIds!.includes(orgId)) return sum;
          return sum + horas;
        }, 0);
...
app.get('/activities', authMiddleware, async (c) => {
  try {
	    ...
	    const mineRaw = String(c.req.query('mine') || '').trim().toLowerCase();
	    const onlyMine = mineRaw === '1' || mineRaw === 'true' || mineRaw === 'yes';
	    ...
	    if (sessionRole === 'trabajador') {
	      if (onlyMine) {
	        ...
	        if (teamError) {
	          throw new Error(`Error al resolver equipo de actividades: ${teamError.message}`);
	        }

	        const teamIds = (teamUsers || [])
	          .map((row: any) => Number(row.id_usuario))
	          .filter((id) => Number.isInteger(id) && id > 0);
	        ...
	        query = query.in('id_responsable', teamIds);
	      }
	    }
...
app.put('/activities/:id/status', authMiddleware, async (c) => {
  try {
    ...
    if (!['admin', 'principal'].includes(sessionRole)) {
...
app.get('/reports/certificate/:volunteerId', authMiddleware, async (c) => {
  try {
    ...
    if (!['admin', 'principal', 'trabajador', 'voluntario'].includes(actorRole)) {
      return c.json({ error: 'Sin permisos para generar certificado' }, 403);
    }

    if (actorRole === 'voluntario' && actorIdInt !== volunteerIdInt) {
      return c.json({ error: 'Sin permisos para generar certificado de otro voluntario' }, 403);
    }
    ...
    const { data: volunteer, error: volunteerError } = await supabase
      .from('usuarios')
      .select('id_usuario, nombre_completo, dni, correo, telefono, organizacion, id_organizacion, id_estado, id_rol, roles(id_rol, nombre)')
      .eq('id_usuario', volunteerIdInt)
      .eq('id_rol', ROLE_IDS.voluntario)
      .maybeSingle();
    ...
    if (actorRole === 'trabajador') {
      if (actorOrgIds.length === 0) return c.json({ error: 'Usuario sin organizacion asignada' }, 403);
      if (volunteerOrgIds.length === 0 || !volunteerOrgIds.some((id) => actorOrgIds.includes(id))) {
        return c.json({ error: 'Sin permisos para generar certificado de otra organizacion' }, 403);
      }
    }
    ...
    if (responsibleIds.length > 0) {
      ...
      for (const responsible of responsables || []) {
        ...
        const orgIds = parseIdListFromCsv((responsible as any).id_organizacion);
        responsibleOrgMap.set(String(responsible.id_usuario), orgIds.length > 0 ? orgIds[0] : null);
      }
    }
    ...
    if (orgIdsToLookup.length > 0) {
      ...
      if (orgErr) {
        console.warn('No se pudo resolver organizaciones en certificado:', orgErr.message);
      } else {
        for (const row of orgRows || []) {
          const id = Number((row as any).id_organizacion);
          const nombre = String((row as any).nombre || '').trim();
    ...
    const activities = (dbActivities || []).map((activity: any) => {
      const respOrg = activity.id_responsable ? (responsibleOrgMap.get(String(activity.id_responsable)) ?? null) : null;
      const orgId = respOrg ?? fallbackOrgId;
      const orgName = orgId ? (orgNameMap.get(orgId) || null) : null;
      ...
      if (!orgRestricted) return true;
      const orgId = Number(row?.id_organizacion);
      return Number.isInteger(orgId) && allowedOrgIds!.includes(orgId);
    });

    const validActivityIds = new Set((activities || []).map((activity: any) => String(activity.id)));
...
app.post('/kobo/sync/voluntario', authMiddleware, async (c) => {
  try {
    ...
    const { data: dbUser, error: userErr } = await supabase
      .from('usuarios')
      .select('id_usuario, dni, id_rol, id_organizacion')
    ...
    if (!['admin', 'principal', 'trabajador', 'voluntario'].includes(actorRole)) {
      return c.json({ success: false, error: 'Sin permisos para sincronizar KoBo' }, 403);
    }

    if (actorRole === 'voluntario' && actorIdInt !== idUsuario) {
      return c.json({ success: false, error: 'Sin permisos para sincronizar otro voluntario' }, 403);
    }

    const volunteerOrgIds = parseIdListFromCsv((dbUser as any).id_organizacion);
    if (actorRole === 'trabajador') {
      if (actorOrgIds.length === 0) return c.json({ success: false, error: 'Usuario sin organizacion asignada' }, 403);
      if (volunteerOrgIds.length === 0 || !volunteerOrgIds.some((id) => actorOrgIds.includes(id))) {
        return c.json({ success: false, error: 'Sin permisos para sincronizar voluntarios de otra organizacion' }, 403);
      }
    }
...
app.get('/voluntarios', authMiddleware, async (c) => {
  try {
    ...
    if (!['admin', 'principal', 'trabajador'].includes(actorRole)) {
      return c.json({
        success: false,
        error: 'Sin permisos para listar voluntarios',
      }, 403);
    }
    ...
    const { data: voluntarios, error } = await query.order('id_usuario', { ascending: false });
...
app.get('/responsables', authMiddleware, async (c) => {
  try {
    ...
    if (!['admin', 'principal', 'trabajador'].includes(actorRole)) {
      return c.json({ success: false, error: 'Sin permisos para listar responsables' }, 403);
    }
    ...
    let query = supabase
      .from('usuarios')
      .select('id_usuario, nombre_completo, correo, id_rol, id_estado, id_organizacion, roles(id_rol, nombre)')
      .in('id_rol', [ROLE_IDS.admin, ROLE_IDS.principal, ROLE_IDS.trabajador])
      .eq('id_estado', activeGeneralId || 1);
    ...
    const { data: responsables, error } = await query.order('nombre_completo');
...
app.post('/actividades', authMiddleware, async (c) => {
  try {
    ...
    if (!['admin', 'principal', 'trabajador'].includes(sessionRole)) {
      return c.json({ success: false, error: 'Sin permisos para crear actividades' }, 403);
    }
...
app.put('/actividades/:id_actividad', authMiddleware, async (c) => {
  try {
    ...
    if (sessionRole === 'trabajador') {
      const parsedRequested = Number.isInteger(requestedEstadoId as any) ? Number(requestedEstadoId) : null;
      if (parsedRequested !== null && parsedRequested > 0 && Number(existingActivity.id_estado) !== parsedRequested) {
        return c.json({ success: false, error: 'Sin permisos para cambiar el estado de la actividad' }, 403);
      }
    }
...
app.get('/actividades/:id_actividad/detalle', authMiddleware, async (c) => {
  ...
  try {
    ...
    if (sessionRole === 'trabajador') {
      const actorOrgIds = resolveOrganizationIdsFromUser(user);
      if (actorOrgIds.length === 0) {
        return c.json({ success: false, error: 'Usuario sin organizacion asignada' }, 403);
      }

      const idsToCheck = Array.from(new Set(
        [Number(actividad.id_responsable), Number(actividad.id_creador)]
          .filter((id) => Number.isInteger(id) && id > 0),
      ));
      ...
      const { data: orgRows, error: orgError } = await supabase
        .from('usuarios')
        .select('id_usuario, id_organizacion')
        .in('id_usuario', idsToCheck);

      if (orgError) {
        throw new Error(`Error validando organizacion de actividad: ${orgError.message}`);
      }

      const activityOrgIds = (orgRows || [])
        .flatMap((row: any) => parseIdListFromCsv(row?.id_organizacion))
        .filter((id) => Number.isInteger(id) && id > 0);

      const isAllowed = activityOrgIds.some((id) => actorOrgIds.includes(id));
      if (!isAllowed) {
        return c.json({ success: false, error: 'Sin permisos para ver esta actividad' }, 403);
      }
    }
...
app.get('/actividades/:id_actividad/evidencias', authMiddleware, async (c) => {
  try {
    ...
    if (!['admin', 'principal', 'trabajador'].includes(actorRole)) {
      return c.json({ success: false, error: 'Sin permisos para ver evidencias' }, 403);
    }
    ...
    if (actorRole === 'trabajador') {
      ...
      if (actError) {
        throw new Error(`Error al validar actividad: ${actError.message}`);
      }
      if (!actividad) {
        return c.json({ success: false, error: 'Actividad no encontrada' }, 404);
      }
      ...
      const idsToCheck = Array.from(new Set(
        [Number((actividad as any).id_responsable), Number((actividad as any).id_creador)]
          .filter((id) => Number.isInteger(id) && id > 0),
      ));
      ...
      const isAllowed = activityOrgIds.some((id) => actorOrgIds.includes(id));
      if (!isAllowed) {
        return c.json({ success: false, error: 'Sin permisos para ver evidencias de otra organizacion' }, 403);
      }
    }
...
app.post('/actividades/:id_actividad/evidencias', authMiddleware, async (c) => {
  try {
    ...
    if (!['admin', 'principal', 'trabajador'].includes(actorRole)) {
      return c.json({ success: false, error: 'Sin permisos para subir evidencias' }, 403);
    }
    ...
    if (actorRole === 'trabajador') {
      ...
      if (Number((actividad as any).id_responsable) !== actorIdInt && Number((actividad as any).id_creador) !== actorIdInt) {
        return c.json({ success: false, error: 'Sin permisos para subir evidencias a esta actividad' }, 403);
      }
    }
...
app.get('/actividad-voluntarios', authMiddleware, async (c) => {
  try {
    ...
    if (!['admin', 'principal', 'trabajador'].includes(actorRole)) {
      return c.json({ success: false, error: 'Sin permisos para ver relaciones' }, 403);
    }
    ...
    if (actorRole === 'trabajador') {
      if (actorOrgIds.length === 0) return c.json({ success: false, error: 'Usuario sin organizacion asignada' }, 403);
      query = query.in('usuarios.id_organizacion', actorOrgIds);
    }

    const { data: relaciones, error } = await query;
...
app.put('/actividad-voluntarios/horas', authMiddleware, async (c) => {
  try {
    ...
    if (!['admin', 'principal', 'trabajador'].includes(actorRole)) {
      return c.json({ success: false, error: 'Sin permisos para actualizar horas' }, 403);
    }
    ...
    if (actorRole === 'trabajador') {
      ...
      const volunteerIdInt = Number(id_usuario);
      const { data: volunteerRow, error: volunteerErr } = await supabase
        .from('usuarios')
        .select('id_usuario, id_rol, id_organizacion')
        .eq('id_usuario', volunteerIdInt)
        .eq('id_rol', ROLE_IDS.voluntario)
      ...
      if (volunteerErr) {
        throw new Error(`Error validando voluntario: ${volunteerErr.message}`);
      }
      ...
      const volunteerOrgIds = parseIdListFromCsv((volunteerRow as any).id_organizacion);
      if (volunteerOrgIds.length === 0 || !volunteerOrgIds.some((id) => actorOrgIds.includes(id))) {
        return c.json({ success: false, error: 'Sin permisos para actualizar horas de otra organizacion' }, 403);
      }
    }
...
app.post('/voluntarios', authMiddleware, async (c) => {
  try {
    ...
    const currentRole = getSessionRoleName(currentUser);
    console.log('=== Creando nuevo voluntario ===');
    console.log('Datos recibidos:', body);

    if (!['admin', 'principal', 'trabajador'].includes(currentRole)) {
      return c.json({ success: false, error: 'Sin permisos para crear voluntarios' }, 403);
    }
