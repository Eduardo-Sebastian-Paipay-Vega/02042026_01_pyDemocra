...
const corsAllowedOrigins = (Deno.env.get('CORS_ALLOWED_ORIGINS') || 'http://localhost:3000')
  .split(',')
  .map((value) => value.trim())
  .filter(Boolean);

// Middleware - CORS con lista explicita de origenes permitidos
app.use('*', cors({
...
app.get('/make-server-7052c263/health', async (c) => {
...
app.get('/make-server-7052c263/test-usuarios', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.post('/make-server-7052c263/auth/signup', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.post('/make-server-7052c263/auth/login', async (c) => {
  try {
    const body = await c.req.json();
...
app.post('/make-server-7052c263/auth/debug-headers', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.post('/make-server-7052c263/auth/test-jwt', authMiddleware, requireRole('admin'), async (c) => {
  try {
    const accessToken = c.req.header('X-Access-Token');
...
app.get('/make-server-7052c263/auth/session', authMiddleware, async (c) => {
  try {
...
app.get('/make-server-7052c263/me', authMiddleware, async (c) => {
  try {
    const userFromToken = c.get('user') as User;
    const userId = Number(userFromToken.id);
    let sessionUser = mapDbUserToSystemUser(userFromToken);
...
app.get('/make-server-7052c263/auth/check-passwords', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.post('/make-server-7052c263/auth/hash-password', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.post('/make-server-7052c263/auth/hash-all-passwords', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.get('/make-server-7052c263/debug/kv-users', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.get('/make-server-7052c263/debug/test-jwt', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.post('/make-server-7052c263/debug/verify-token', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.get('/make-server-7052c263/debug/jwt-config', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.get('/make-server-7052c263/estados', authMiddleware, async (c) => {
  try {
...
app.get('/make-server-7052c263/roles', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.get('/make-server-7052c263/organizaciones', authMiddleware, requireRole('admin'), async (c) => {
  try {
    const activeGeneralId = await getEstadoIdByName('general', 'activo');
...
const listUsersHandler = async (c: any) => {
  try {
    ...
    const { data: dbUsers, error } = await supabase
      .from('usuarios')
      .select('*, roles(id_rol, nombre), organizaciones(id_organizacion, nombre)')
      .order('id_usuario');
...
app.get('/make-server-7052c263/users', authMiddleware, requireRole('admin'), listUsersHandler);
app.get('/make-server-7052c263/usuarios', authMiddleware, requireRole('admin'), listUsersHandler);

// Crear usuario
app.post('/make-server-7052c263/users', authMiddleware, requireRole('admin'), async (c) => {
  try {
    const actor = c.get('user') as User;
    const body = await c.req.json();
    const {
...
// Actualizar usuario
app.put('/make-server-7052c263/users/:id', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
// Eliminar usuario
app.delete('/make-server-7052c263/users/:id', authMiddleware, requireRole('admin'), async (c) => {
  try {
    const userId = c.req.param('id');
    const userIdInt = Number(userId);
    const actor = c.get('user') as User;
...
// Ãreas
app.get('/make-server-7052c263/legacy/areas', authMiddleware, async (c) => {
  try {
...
app.post('/make-server-7052c263/areas', authMiddleware, requireRole('admin'), async (c) => {
  try {
    const body = await c.req.json();
...
app.put('/make-server-7052c263/areas/:id', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.delete('/make-server-7052c263/areas/:id', authMiddleware, requireRole('admin'), async (c) => {
  try {
    const areaId = Number(c.req.param('id'));
    if (!Number.isInteger(areaId) || areaId <= 0) {
      return c.json({ error: 'id_area invalido' }, 400);
...
// Tipos de actividad
app.get('/make-server-7052c263/activity-types', authMiddleware, async (c) => {
  try {
...
app.post('/make-server-7052c263/activity-types', authMiddleware, requireRole('admin'), async (c) => {
  try {
    const body = await c.req.json();
    const user = c.get('user') as User;
...
app.put('/make-server-7052c263/activity-types/:id', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.delete('/make-server-7052c263/activity-types/:id', authMiddleware, requireRole('admin'), async (c) => {
  try {
    const typeId = Number(c.req.param('id'));
    if (!Number.isInteger(typeId) || typeId <= 0) {
      return c.json({ error: 'id_tipo_actividad invalido' }, 400);
...
// Ubicaciones/Sedes
app.get('/make-server-7052c263/locations', authMiddleware, async (c) => {
  try {
...
app.post('/make-server-7052c263/locations', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
// Listar voluntarios
app.get('/make-server-7052c263/volunteers', authMiddleware, async (c) => {
  try {
    const estadoIndex = await refreshEstadosCache();
...
app.get('/make-server-7052c263/volunteers/:id', authMiddleware, async (c) => {
  try {
...
app.put('/make-server-7052c263/volunteers/:id', authMiddleware, async (c) => {
  try {
    const actor = c.get('user') as User;
...
app.get('/make-server-7052c263/activities', authMiddleware, async (c) => {
  try {
    const user = c.get('user') as User;
    ...
    if (sessionRole === 'admin' || sessionRole === 'principal') {
      // Admin y principal ven todas las actividades
    } else if (sessionRole === 'trabajador') {
      filteredActivities = filteredActivities.filter((activity: any) => Number(activity.id_responsable) === userIdInt);
    } else {
      filteredActivities = [];
    }
...
// Crear actividad
app.post('/make-server-7052c263/activities', authMiddleware, async (c) => {
  try {
    return c.json(
      {
...
app.put('/make-server-7052c263/activities/:id', authMiddleware, async (c) => {
  try {
    return c.json(
      {
...
app.post('/make-server-7052c263/activities/:id/validate', authMiddleware, requireRole('principal'), async (c) => {
  try {
...
app.put('/make-server-7052c263/activities/:id/status', authMiddleware, async (c) => {
  try {
    const user = c.get('user') as User;
    const sessionRole = getSessionRoleName(user);
...
app.get('/make-server-7052c263/reports/certificate/:volunteerId', authMiddleware, async (c) => {
  try {
...
app.get('/make-server-7052c263/reports/metrics', authMiddleware, async (c) => {
  try {
    const user = c.get('user') as User;
    const sessionRole = getSessionRoleName(user);
    const userIdInt = await resolveUserIdInt(user, supabase);
...
app.get('/make-server-7052c263/audit-logs', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.get('/make-server-7052c263/debug', authMiddleware, requireRole('admin'), async (c) => {
  try {
    const users = await kv.getByPrefix<User>('users:');
    const areas = await kv.get<Area[]>('areas');
...
app.post('/make-server-7052c263/init', authMiddleware, requireRole('admin'), async (c) => {
  try {
...
app.get('/make-server-7052c263/kobo/health', authMiddleware, async (c) => {
  try {
...
app.get('/make-server-7052c263/kobo/forms', authMiddleware, async (c) => {
  try {
...
app.get('/make-server-7052c263/kobo/forms/:assetUid', authMiddleware, async (c) => {
  try {
...
app.get('/make-server-7052c263/kobo/forms/:assetUid/submissions', authMiddleware, async (c) => {
  try {
    const assetUid = c.req.param('assetUid');
...
app.post('/make-server-7052c263/kobo/forms/:assetUid/submissions', authMiddleware, async (c) => {
  try {
    const assetUid = c.req.param('assetUid');
...
app.patch('/make-server-7052c263/kobo/forms/:assetUid/submissions/:submissionId', authMiddleware, async (c) => {
  try {
    const assetUid = c.req.param('assetUid');
...
app.delete('/make-server-7052c263/kobo/forms/:assetUid/submissions/:submissionId', authMiddleware, requireRole('admin'), async (c) => {
  try {
    const assetUid = c.req.param('assetUid');
    const submissionId = c.req.param('submissionId');
    const user = c.get('user') as User;
...
app.get('/make-server-7052c263/kobo/asistencia-horas', authMiddleware, async (c) => {
  try {
...
app.post('/make-server-7052c263/kobo/asistencia-horas', authMiddleware, async (c) => {
  try {
    const data = await c.req.json();
    const user = c.get('user') as User;
...
app.get('/make-server-7052c263/kobo/ejecucion-evidencias', authMiddleware, async (c) => {
  try {
...
app.post('/make-server-7052c263/kobo/ejecucion-evidencias', authMiddleware, async (c) => {
  try {
    const data = await c.req.json();
    const user = c.get('user') as User;
 ...
 */
app.post('/make-server-7052c263/sync/asistencia-horas', authMiddleware, async (c) => {
  try {
    const user = c.get('user') as User;
 ...
 */
app.post('/make-server-7052c263/sync/ejecucion-evidencias', authMiddleware, async (c) => {
  try {
    const user = c.get('user') as User;
 ...
 */
app.post('/make-server-7052c263/sync/all', authMiddleware, async (c) => {
  try {
    const user = c.get('user') as User;
 ...
 */
app.get('/make-server-7052c263/legacy/actividades/generar-codigo', authMiddleware, async (c) => {
  try {
 ...
 */
app.get('/make-server-7052c263/sync/stats', authMiddleware, async (c) => {
  try {
 ...
 */
app.get('/make-server-7052c263/areas', authMiddleware, async (c) => {
  try {
 ...
 */
app.get('/make-server-7052c263/voluntarios', authMiddleware, async (c) => {
  try {
 ...
 */
app.get('/make-server-7052c263/tipos-actividad', authMiddleware, async (c) => {
  try {
 ...
 */
app.get('/make-server-7052c263/responsables', authMiddleware, async (c) => {
  try {
    ...
    const { data: responsables, error } = await supabase
      .from('usuarios')
      .select('id_usuario, nombre_completo, id_rol, id_estado, roles(id_rol, nombre)')
      .in('id_rol', [ROLE_IDS.admin, ROLE_IDS.principal, ROLE_IDS.trabajador])
      .eq('id_estado', activeGeneralId || 1)
      .order('nombre_completo');
 ...
 */
app.get('/make-server-7052c263/actividades/generar-codigo', authMiddleware, async (c) => {
  try {
 ...
 */
app.post('/make-server-7052c263/actividades', authMiddleware, async (c) => {
  try {
    const body = await c.req.json();
    const user = c.get('user') as User;
    const sessionRole = getSessionRoleName(user);
 ...
 * Actualizar actividad
 */
app.put('/make-server-7052c263/actividades/:id_actividad', authMiddleware, async (c) => {
  try {
 ...
 * Eliminar actividad
 */
app.delete('/make-server-7052c263/actividades/:id_actividad', authMiddleware, async (c) => {
  try {
    const id_actividad = c.req.param('id_actividad');
    const actividadId = Number(id_actividad);
...
app.get('/make-server-7052c263/actividades/:id_actividad/evidencias', authMiddleware, async (c) => {
  try {
    const id_actividad = c.req.param('id_actividad');
    const actividadId = Number(id_actividad);
...
app.post('/make-server-7052c263/actividades/:id_actividad/evidencias', authMiddleware, async (c) => {
  try {
    const id_actividad = c.req.param('id_actividad');
    const actividadId = Number(id_actividad);
 ...
 */
app.get('/make-server-7052c263/actividad-voluntarios', authMiddleware, async (c) => {
  try {
 ...
 */
app.put('/make-server-7052c263/actividad-voluntarios/horas', authMiddleware, async (c) => {
  try {
    const body = await c.req.json();
 ...
 */
app.post('/make-server-7052c263/voluntarios', authMiddleware, async (c) => {
  try {
    const body = await c.req.json();
