...
export function BeneficiaryFormModal({
  ...
  const submit = handleSubmit(async (values) => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      setSubmitError(
        validationErrors.childProfile ?? "Revisa los campos obligatorios del beneficiario."
      );
      return;
    }
    ...
    try {
      await onSubmit(await adaptBeneficiaryFormToUpsertInput(values));
    } catch (error) {
      setSubmitError(
        error instanceof Error ? error.message : "No se pudieron subir los archivos del beneficiario."
      );
      return;
    } finally {
