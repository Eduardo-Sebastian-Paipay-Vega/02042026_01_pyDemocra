...
function buildProjectPayload(
  ...
  if (!code) {
    throw new Error("El codigo del proyecto no pudo generarse.");
  }
  if (!name) {
...
export async function createProject(input: ProjectFormValues): Promise<ProjectDetailData> {
  try {
    ...
    const inputWithCode: ProjectFormValues = {
      ...input,
      code: input.code?.trim() || (await generateProjectCode(tenantId)),
    };
    const payload = buildProjectPayload(inputWithCode, actorId, tenantId);
