...
export async function createAccessLink(input: CreateAccessLinkInput): Promise<AccessLinkRow> {
  ...
  // Genera un código aleatorio URL-safe (12 bytes -> base64url sin padding).
...
// ──────────────────────────────────────────────────────────────
// ROLES / SEDES (para el selector del formulario de creación)
// ──────────────────────────────────────────────────────────────

export async function listAssignableRoles(): Promise<RoleRow[]> {
  const tenantId = await getRequiredTenantId();
  const { data, error } = await publicSchema()
    .from("roles")
    .select("*")
...
export async function listAssignableSedes(): Promise<SedeRow[]> {
  const tenantId = await getRequiredTenantId();
  const { data, error } = await publicSchema()
    .from("sedes")
    .select("*")
...
// ──────────────────────────────────────────────────────────────
// VALIDACIÓN Y CONSUMO DE CÓDIGO (público — sin tenant requerido)
// ──────────────────────────────────────────────────────────────
...
export interface CompleteAccessOnboardingResult {
  success: boolean;
  membershipId?: string;
  entityId?: string;
  tenantId?: string;
...
/**
 * Consume un código de acceso ya autenticado: crea/actualiza profile,
 * membresía (con el rol del link, si tiene uno asignado) y el registro
 * operativo correspondiente (voluntario/solicitud). Debe llamarse DESPUÉS
 * de que el usuario tenga una sesión activa (signUp/signIn ya resuelto).
 */
export async function completeAccessOnboarding(
  code: string,
  metadata: Record<string, unknown> = {}
): Promise<CompleteAccessOnboardingResult> {
  ...
  return {
    ...
    membershipId: typeof payload.membership_id === "string" ? payload.membership_id : undefined,
    ...
    linkType: typeof payload.link_type === "string" ? payload.link_type : undefined,
  };
}
