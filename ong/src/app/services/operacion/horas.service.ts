...
export async function resolveHoras(input: HoursResolutionInput): Promise<MutationFeedback> {
  try {
    ...
    if (
      (databaseState === "aprobada" || databaseState === "rechazada") &&
      current.created_by
    ) {
      const estadoLabel = databaseState === "aprobada" ? "aprobado" : "rechazado";
      await createInAppNotification({
        tenantId,
        recipientId: current.created_by,
        titulo: `Registro de horas ${estadoLabel}`,
        mensaje: comment
          ? `Tu registro de horas fue ${estadoLabel}. Comentario: ${comment}`
          : `Tu registro de horas fue ${estadoLabel}.`,
        actorId,
      }).catch(() => undefined);
    }
