...
export async function createEvidencia(
  ...
  try {
    ...
    const requestedBy = volunteerId ?? actorId;

    const { error: approvalError } = await ongSchema()
      .from("aprobaciones")
      .insert({
        tenant_id: tenantId,
        modulo: "operation",
        ...
        entidad_id: evidenceId,
        tipo_aprobacion: "evidencia",
        estado: "pendiente",
    ...
    return {
      id: evidenceId,
      approvalSynced: !approvalError,
      warning: approvalError
        ? `Evidencia registrada, pero no se pudo crear la aprobacion: ${approvalError.message}`
        : undefined,
    };
  } catch (error) {
