...
export function VolunteerFormModal({
  ...
  const submit = handleSubmit(async (values) => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      setSubmitError(
        validationErrors.skills ??
          ...
          validationErrors.coordinator ??
          "Revisa los campos obligatorios del voluntario."
      );
      return;
    }
    ...
    try {
      await onSubmit(await adaptVolunteerFormToUpsertInput(values));
    } catch (error) {
      setSubmitError(
        error instanceof Error ? error.message : "No se pudieron subir los archivos del voluntario."
      );
      return;
    } finally {
          ...
          <div className="space-y-3">
            ...
            {hasCoordinatorProfile && (
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <PeopleField label="Anios experiencia">
                  <PeopleTextInput
                    type="number"
                    min={0}
