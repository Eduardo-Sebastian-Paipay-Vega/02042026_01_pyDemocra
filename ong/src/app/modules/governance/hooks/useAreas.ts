...
import { AreaRow, listAreas, createArea, updateArea, toggleAreaActive } from "../../../services/gobernanza/areas.service";
...
export function useAreas(searchTerm: string) {
  ...
  const handleCreate = async (payload: { code: string; name: string; description: string; active: boolean }) => {
    try {
  ...
  const handleUpdate = async (id: string, payload: { code: string; name: string; description: string; active: boolean }) => {
    try {
  ...
  const handleToggleStatus = async (id: string, currentStatus: boolean) => {
    try {
      await toggleAreaActive(id, !currentStatus);
