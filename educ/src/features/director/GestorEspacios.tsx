...
// RF-046: Gestor de Espacios Físicos Inteligente
...
const ESPACIOS = [
  ...
  { id: 'lab1', type: 'lab', name: 'Lab. Física', capacity: 20, occupied: 22, temp: 24, status: 'overcrowded' },
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Plano arquitectónico vivo. Optimización de aforos y alertas de superposición.
          </p>
        </div>
 ...
 ${espacio.status === 'overcrowded' ? 'text-rose-400' : ''}`}>{espacio.name}</h4>
                ...
                <div className="flex items-center gap-4 text-sm mt-3">
                  <div className="flex flex-col">
                    <span className="text-slate-500 text-xs">Ocupación</span>
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-slate-200 font-medium">{espacio.temp}°C</span>
                  </div>
                </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-rose-950/20 border border-rose-500/20 rounded-2xl p-5">
            <h3 className="font-bold text-rose-500 flex items-center gap-2 mb-3">
              <AlertTriangle className="w-4 h-4" /> Alerta de Superposición
            </h3>
            <p className="text-sm text-rose-200/80 mb-3">
              El <strong>Lab. Física</strong> supera el aforo permitido (22/20) para el periodo actual. Se detectó falta de sillas.
            </p>
            <button className="bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold px-3 py-1.5 rounded transition">
              Reasignar a Lab. Química (Libre)
            </button>
          </div>

          <div className="card p-5">
            <h3 className="font-bold text-white flex items-center gap-2 mb-4">
              <Clock className="w-4 h-4 text-purple-400" /> Próximas Asignaciones
            </h3>
            <div className="space-y-3">
              <div className="flex justify-between items-center p-3 bg-white/5 rounded-xl border border-border">
                <div>
                  <div className="text-sm font-bold text-slate-200">Reunión Docente</div>
