...
// RF-051: Motor de ExÃ¡menes Adaptativos CAT/IRT

export const EngineCATIRT = () => {
  const [theta, setTheta] = useState(0.5); // Nivel de maestrÃ­a estimado
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 card p-6 rounded-2xl">
        <div>
          <h2 className="text-2xl font-bold text-white flex items-center gap-2">
            ...
            EvaluaciÃ³n DiagnÃ³stica Adaptativa
          </h2>
          <p className="text-muted-foreground mt-1 text-sm">
            Motor CAT/IRT: La dificultad se ajusta en tiempo real segÃºn tus respuestas.
          </p>
        </div>
 ...
 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'}`}
                  >
                    Nivel {difficulty > 0.7 ? 'DifÃ­cil' : difficulty > 0.4 ? 'Medio' : 'FÃ¡cil'}
                  </span>
                </div>

                <h3 className="text-2xl font-medium text-white mb-8 leading-relaxed">
                  Â¿CuÃ¡l es la derivada de <span className="font-mono text-indigo-300 bg-indigo-500/10 px-2 py-1 rounded">f(x) = {difficulty > 0.7 ? 'e^(2x) * sin(x)' : 'x^2 + 3x'}</span>?
                </h3>
                ...
                <div className="space-y-3">
                  {[1, 2, 3, 4].map((opt) => (
                    ...
                    >
                      ...
                      OpciÃ³n matemÃ¡tica de respuesta {opt}
                    </button>
                  ))}
                </div>
              </motion.div>
 ...
 ${showResult === 'correct' ? 'border-emerald-500/50 shadow-[0_0_30px_rgba(16,185,129,0.1)]' : 'border-rose-500/50 shadow-[0_0_30px_rgba(244,63,94,0.1)]'}`}
              >
                ...
                <h3 className="text-2xl font-bold text-white mb-2">
                  {showResult === 'correct' ? 'Â¡Correcto!' : 'Incorrecto'}
                </h3>
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="card p-5">
            <h3 className="font-bold text-white flex items-center gap-2 mb-4">
              <TrendingUp className="w-4 h-4 text-indigo-400" /> Progreso de PrecisiÃ³n
            </h3>
            
            <div className="space-y-4">
              <div className="flex flex-col gap-1">
                <div className="flex justify-between text-xs text-slate-400">
                  <span>Error EstÃ¡ndar (SE)</span>
                  <span>{Math.max(0.05, 0.4 - (questionNum * 0.02)).toFixed(3)}</span>
                </div>
                <div className="text-[10px] text-indigo-300">
                  El examen termina automÃ¡ticamente cuando el SE es menor a 0.15, garantizando precisiÃ³n sin sobre-evaluar.
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-indigo-950/20 border border-indigo-500/20 p-4 rounded-xl">
            <p className="text-xs text-indigo-200">
              <strong>Info:</strong> EstÃ¡s en un entorno de evaluaciÃ³n CAT. No se puede retroceder a preguntas anteriores.
            </p>
          </div>
        </div>
