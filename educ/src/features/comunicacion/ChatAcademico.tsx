...
// RF-011: Chat AcadÃ©mico Supervisado

const INITIAL_MESSAGES = [
  { id: 1, sender: 'teacher', text: 'Â¡Hola a todos! Recuerden que el trabajo de Ãlgebra se entrega maÃ±ana.', time: '09:00 AM' },
  { id: 2, sender: 'student', text: 'Profesor, Â¿podemos usar la fÃ³rmula de Bhaskara?', time: '09:05 AM' },
  { id: 3, sender: 'teacher', text: 'SÃ­, es correcto. Les dejo un apunte.', time: '09:10 AM' },
  { id: 4, sender: 'ai', text: 'ðŸ›¡ï¸ Mensaje automÃ¡tico: El chat ha sido moderado a las 10:00 AM para prevenir lenguaje inapropiado.', time: '10:00 AM', isSystem: true },
];

export const ChatAcademico = () => {
  const [messages, setMessages] = useState(INITIAL_MESSAGES);
  const [input, setInput] = usePersistedState('chat_comunicacion_input', '');
    ...
    <div className="flex flex-col h-[calc(100vh-140px)] max-h-[800px] card overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-black/40 border-b border-border p-4 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-bold leading-tight">Ãlgebra MatemÃ¡tica 4to B</h3>
            <p className="text-xs text-emerald-400 flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-emerald-500 inline-block" /> 35 en lÃ­nea
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 text-xs text-indigo-400 bg-indigo-500/10 px-3 py-1.5 rounded-full border border-indigo-500/20">
          <Bot className="w-4 h-4" /> ModeraciÃ³n IA Activa
        </div>
      </div>
