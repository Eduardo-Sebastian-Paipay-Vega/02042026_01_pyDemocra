...
const radarData = new Proxy([] as any, {
  get: (target, prop) => {
    const realData = (window as any).__dashboardData?.radarData;
    if (!realData) return undefined;
    const value = realData[prop];
    return typeof value === 'function' ? value.bind(realData) : value;
  }
});
const comunicaciones = new Proxy([] as any, {
  get: (target, prop) => {
    const realData = (window as any).__dashboardData?.comunicaciones;
    if (!realData) return undefined;
    const value = realData[prop];
    return typeof value === 'function' ? value.bind(realData) : value;
  }
...
export default function DashboardPadres({ view }: { view: string }) {
  ...
  useEffect(() => {
    fetchEducData('analytics/padres').then(d => {
