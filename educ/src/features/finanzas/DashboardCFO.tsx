...
import { TrendingUp, TrendingDown, DollarSign, Users, AlertTriangle, RefreshCw, Bot, CheckCircle2, XCircle, Clock, ArrowUpRight, ArrowDownRight, Download, Search, Filter, ChevronRight, Building2 } from 'lucide-react'
...
const canalesPago = new Proxy([] as any, {
  get: (target, prop) => {
    const realData = (window as any).__dashboardData?.canalesPago;
    if (!realData) return undefined;
    const value = realData[prop];
    return typeof value === 'function' ? value.bind(realData) : value;
  }
});
const flujoCaja = new Proxy([] as any, {
  get: (target, prop) => {
    const realData = (window as any).__dashboardData?.flujoCaja;
    if (!realData) return undefined;
    const value = realData[prop];
    return typeof value === 'function' ? value.bind(realData) : value;
  }
...
const deudorScore = new Proxy([] as any, {
  get: (target, prop) => {
    const realData = (window as any).__dashboardData?.deudorScore;
    if (!realData) return undefined;
    const value = realData[prop];
    return typeof value === 'function' ? value.bind(realData) : value;
  }
...
const transacciones = new Proxy([] as any, {
  get: (target, prop) => {
    const realData = (window as any).__dashboardData?.transacciones;
    if (!realData) return undefined;
    const value = realData[prop];
    return typeof value === 'function' ? value.bind(realData) : value;
  }
...
export default function DashboardCFO({ view }: { view: string }) {
  ...
  useEffect(() => {
    fetchEducData('analytics/cfo').then(d => {
  ...
  return (
    <div style={{ padding:24 }} className="fade-up">
      <PageHeader title="Finanzas" sub="Tiempo real · CFO Dashboard"
      ...
      <div style={{ display:'grid', gridTemplateColumns:'260px 1fr', gap:12 }}>
        ...
        <div className="card" style={{ padding:16 }}>
          ...
          {[
            { texto:'Ofrecer plan BNPL a Familia López (Score 65/100)', urgencia:'media' },
            { texto:'Ejecutar cobranza automática: Familia García (90+ días mora)', urgencia:'alta' },
            { texto:'Plan de pagos extendido: Familia Martínez', urgencia:'baja' },
          ].map((r,i) => (
...
function Transacciones() {
  ...
  return (
    <div style={{ padding: 24 }} className="fade-up">
      <PageHeader title="Transacciones" sub="Historial de pagos recibidos · Mayo 2026"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 20 }}>
        {[
          ...
          { label: 'Fallidas',           value: `${transacciones.filter(t=>t.estado==='fallido').length}`, sub: 'Requieren atención' },
        ].map(k => (
      ...
      <div className="card">
        ...
        <table className="table">
          <thead><tr><th>ID</th><th>Familia</th><th>Concepto</th><th>Fecha</th><th>Método</th><th>Monto</th><th>Estado</th><th></th></tr></thead>
          <tbody>
            {filtradas.map(t => (
              <tr key={t.id}>
...
function DeudoresCFO() {
  ...
  return (
    <div style={{ padding: 24 }} className="fade-up">
      <PageHeader title="Deudores" sub="Gestión de morosidad por scoring de riesgo · Mayo 2026"
        actions={
          <>
            ...
            <div key={d.familia} className="card" style={{ padding: 16, borderColor: d.score==='alto' ? 'rgba(239,68,68,0.2)' : d.score==='medio' ? 'rgba(245,158,11,0.2)' : 'var(--border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--tx-3)', display: 'flex', gap: 16 }}>
                    ...
                    <span>Mora: <strong style={{ color: 'var(--tx-2)' }}>{d.dias} días</strong></span>
                  </div>
                </div>
                {acc ? (
              ...
              {d.score !== 'bajo' && !acc && (
                <div style={{ marginTop: 10, padding: '8px 12px', borderRadius: 'var(--radius)', background: 'var(--s3)', fontSize: 12, color: 'var(--tx-3)', borderLeft: `3px solid ${d.score==='alto'?'var(--red)':'var(--amber)'}` }}>
                  <strong style={{ color: 'var(--tx-2)' }}>Recomendación IA:</strong>{' '}
                  {d.score === 'alto'
                    ? `Ejecutar cobranza automática + llamada de seguimiento. Última respuesta hace ${d.dias} días.`
                    : `Ofrecer plan BNPL en 6 cuotas. Alta probabilidad de aceptación (score 65+).`}
                </div>
              )}
            </div>
          )
...
function AgentesIA() {
  ...
  return (
    <div style={{ padding:24 }} className="fade-up">
      <PageHeader title="Agentes IA" sub="Supervisión de agentes autónomos en ejecución"
