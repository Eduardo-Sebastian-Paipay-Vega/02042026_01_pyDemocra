...
import { useSettings, type Density, type Theme, type FontSize, type AppSettings } from '@/context/SettingsContext'
import { type RGBColor, rgbToHex, hexToRgb, getContrastText, rgbToCss, applyColorVars } from '@/utils/color'
...
function loadColors(): Record<ColorKey, RGBColor> {
  try {
    const raw = localStorage.getItem('institution-colors')
    if (raw) return { ...COLOR_DEFAULTS, ...JSON.parse(raw) }
  } catch {}
...
export default function SettingsPage() {
  ...
  const saveColors = () => {
    localStorage.setItem('institution-colors', JSON.stringify(colorDraft))
