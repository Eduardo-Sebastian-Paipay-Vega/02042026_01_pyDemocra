...
type DocumentoInsertPayload = AppDatabase["rrhh"]["Tables"]["documentos_admision"]["Insert"];
type DocumentoUpdatePayload = AppDatabase["rrhh"]["Tables"]["documentos_admision"]["Update"];
type OnboardingVoluntarioInsertPayload =
  AppDatabase["rrhh"]["Tables"]["onboarding_voluntario"]["Insert"];
type OnboardingVoluntarioUpdatePayload =
  AppDatabase["rrhh"]["Tables"]["onboarding_voluntario"]["Update"];
...
function resolveStatusCodesForFilter(status: "all" | AdmissionStateKind): string[] | null {
  if (status === "all") {
    return null;
  }
  if (status === "received") {
...
async function fetchVolunteersByIds(
  ...
  const uniqueIds = Array.from(
    new Set(volunteerIds.map((value) => sanitizeOptionalId(value)).filter((id): id is string => Boolean(id)))
  );
...
async function fetchProfileLabelsByIds(
  ...
  const uniqueIds = Array.from(
    new Set(userIds.map((value) => sanitizeOptionalId(value)).filter((id): id is string => Boolean(id)))
  );
...
function buildDocumentoAdmisionInsertPayload(options: {
  tenantId: string;
  requestId: string;
  type: string;
  fileUrl: string;
...
function buildDocumentoAdmisionUpdatePayload(options: {
  actorId: string | null;
  ...
  if (typeof options.type === "string") {
    payload.tipo_documento = sanitizeText(options.type, 50);
  }
...
function buildOnboardingVoluntarioPayload(options: {
  actorId: string | null;
  completed: boolean;
  evidenceUrl?: string | null;
}): OnboardingVoluntarioUpdatePayload {
  const payload: OnboardingVoluntarioUpdatePayload = {
    completado: options.completed,
    fecha_completado: options.completed ? new Date().toISOString() : null,
    updated_by: options.actorId,
    is_deleted: false,
    deleted_at: null,
    deleted_by: null,
  ...
  if (Object.prototype.hasOwnProperty.call(options, "evidenceUrl")) {
    payload.evidencia_url = sanitizeText(options.evidenceUrl ?? null, 2000) || null;
  }
...
export async function getAdmissionReferenceCatalogs(): Promise<AdmissionReferenceCatalogs> {
  const [documentTypes, genders, countries, volunteerStates] = await Promise.all([
    ...
    loadCatalogRows<VolunteerStateCatalogRow>(
      async () =>
        ongSchema()
  ...
  return {
    ...
    volunteerStates: volunteerStates.map((item) => ({
      value: item.codigo,
...
export async function listSolicitudes(
  ...
  try {
    ...
    if (statusCodes && statusCodes.length > 0) {
      ...
      baseQuery = baseQuery.in("estado", statusCodes as any);
    }
    ...
    if (statusCodes && statusCodes.length > 0) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      pageQuery = pageQuery.in("estado", statusCodes as any);
    }
...
export async function getSolicitudAdmisionDetail(
  ...
  try {
    ...
    const verifierLabels = await fetchProfileLabelsByIds(
      tenantId,
      documents.map((item) => item.verified_by).filter((id): id is string => Boolean(id))
    );

    const onboardingByStepId = new Map(onboardingRows.map((item) => [item.id_paso, item]));
    const onboardingSteps = relatedVolunteer
      ? onboardingStepsCatalog.map((step) => mapOnboardingRow(step, relatedVolunteer, onboardingByStepId.get(step.id)))
      : [];
...
export async function listDocumentosBySolicitud(
  ...
  try {
    ...
    const verifierLabels = await fetchProfileLabelsByIds(
      tenantId,
      documents.map((row) => row.verified_by).filter((id): id is string => Boolean(id))
    );
...
export async function createDocumentoAdmision(
  ...
  try {
    ...
    const { data, error } = await rrhhSchema()
      .from("documentos_admision")
      .insert(
        buildDocumentoAdmisionInsertPayload({
          tenantId,
          requestId,
          type,
          fileUrl,
    ...
    const verifierLabels = await fetchProfileLabelsByIds(
      tenantId,
      [(data as DocumentoRow).verified_by].filter((id): id is string => Boolean(id))
    );
...
export async function updateDocumentoAdmision(input: {
  ...
  try {
    ...
    const payload = buildDocumentoAdmisionUpdatePayload({
      actorId,
      existingRow,
      type: input.type,
  ...
  try {
    ...
    const { data, error } = await rrhhSchema()
      ...
      .insert({
        ...
        resultado: result as "apto" | "no_apto" | "pendiente",
        puntaje: score,
        created_by: actorId,
        updated_by: actorId,
      })
    ...
    const request = await fetchRequestById(tenantId, requestId);
    const resolvedInterviewerId = interviewerId ?? "";
    const interviewerLabels = await fetchProfileLabelsByIds(tenantId, resolvedInterviewerId ? [resolvedInterviewerId] : []);
    return mapInterviewRow(
      data as EntrevistaRow,
      request ? `${request.nombres} ${request.apellidos}`.trim() : requestId,
      interviewerLabels.get(resolvedInterviewerId) ?? resolvedInterviewerId
    );
  } catch (error) {
...
export async function listOnboardingBySolicitud(
  ...
  try {
    ...
    const onboardingByStepId = new Map(onboardingRows.map((item) => [item.id_paso, item]));
    return steps.map((step) => mapOnboardingRow(step, relatedVolunteer, onboardingByStepId.get(step.id)));
  } catch (error) {
  ...
  try {
    ...
    if (missingSteps.length > 0) {
      const { error } = await rrhhSchema().from("onboarding_voluntario").insert(
        missingSteps.map<OnboardingVoluntarioInsertPayload>((step) => ({
          tenant_id: tenantId,
    ...
    for (const step of deletedSteps) {
      ...
      const { error } = await rrhhSchema()
        ...
        .update({
          ...
          deleted_by: null,
          updated_by: actorId,
        } satisfies OnboardingVoluntarioUpdatePayload)
        .eq("tenant_id", tenantId)
        .eq("id", deletedRow.id);
...
export async function toggleOnboardingStep(input: {
  ...
  try {
    ...
    const payload = buildOnboardingVoluntarioPayload({
      actorId,
    ...
    return mapOnboardingRow(step, volunteerSummary, row ?? undefined);
  } catch (error) {
