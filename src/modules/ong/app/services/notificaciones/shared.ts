...
export function toFriendlyError(error: unknown, fallbackMessage: string): string {
  ...
  if (normalized.includes("duplicate") || normalized.includes("unique")) {
    return "Ya existe un registro con esos datos dentro del tenant actual.";
  }
...
export function toDayStartIso(value: string): string {
  return `${value}T00:00:00`;
}

export function toDayEndIso(value: string): string {
...
export async function getRequiredTenantId(): Promise<string> {
  ...
  const { data, error } = await notificationsDb.rpc("fn_current_tenant_id");
  if (error) {
...
export async function resolveCurrentUserId(): Promise<string | null> {
  try {
    const { data } = await notificationsDb.auth.getUser();
...
async function hasPermission(permission: string, warnings: string[]): Promise<boolean> {
  try {
    const { data, error } = await notificationsDb.rpc("fn_has_permission", {
...
async function isTenantAdmin(warnings: string[]): Promise<boolean> {
  try {
    const { data, error } = await notificationsDb.rpc("fn_is_tenant_admin");
    if (error) {
...
export async function resolveNotificationCapabilities(): Promise<NotificationsCapabilityState> {
  ...
  if (!currentUserId) {
    return {
      ...
      warnings: warnings.concat("No se pudo resolver el usuario autenticado."),
    };
  }

  const [admin, canRead, canManage] = await Promise.all([
    isTenantAdmin(warnings),
    hasPermission("notifications.read", warnings),
    hasPermission("notifications.manage", warnings),
  ]);

  const canReadNotifications = admin || canRead || canManage;
  ...
  try {
    ...
    return {
      ...
      tenantId,
      isTenantAdmin: admin,
      hasReadPermission: canRead,
      hasManagePermission: canManage,
      canReadTemplates: canReadNotifications,
      canManageTemplates: admin || canManage,
      canReadHistory: canReadNotifications,
      warnings: warnings.concat(
        "Notificaciones valida `notifications.read` y `notifications.manage` con public.fn_has_permission() y mantiene aislamiento tenant-scoped sobre comunicaciones.*."
      ),
    };
  } catch (error) {
    return {
      ...
      canReadHistory: false,
      warnings: warnings.concat(
        toFriendlyError(error, "No se pudo resolver el tenant actual para Notificaciones.")
      ),
    };
  }
}
