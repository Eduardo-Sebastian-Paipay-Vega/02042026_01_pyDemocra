...
type VolunteerInsertPayload = AppDatabase["ong"]["Tables"]["voluntarios"]["Insert"];
type VolunteerUpdatePayload = AppDatabase["ong"]["Tables"]["voluntarios"]["Update"];
...
async function syncVolunteerSkills(
  ...
  for (const skill of skills) {
    ...
    const payload = {
      ...
      nivel: sanitizeSkillLevel(skill.level) as "basico" | "intermedio" | "avanzado" | "experto" | null,
      updated_by: actorId,
    };
...
async function upsertVolunteerBase(
  ...
  const { data, error } = await ongSchema()
    ...
    .insert({
      ...payload,
      created_by: actorId,
    } satisfies VolunteerInsertPayload)
    .select("id")
    .limit(1);
