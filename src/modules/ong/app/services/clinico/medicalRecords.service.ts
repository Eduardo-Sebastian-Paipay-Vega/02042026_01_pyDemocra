...
type BeneficiaryMedicalInsertPayload = AppDatabase["clinico"]["Tables"]["fichas_medicas"]["Insert"];
type BeneficiaryMedicalUpdatePayload = AppDatabase["clinico"]["Tables"]["fichas_medicas"]["Update"];
type VolunteerSensitiveInsertPayload =
  AppDatabase["clinico"]["Tables"]["ficha_sensible_voluntario"]["Insert"];
type VolunteerSensitiveUpdatePayload =
  AppDatabase["clinico"]["Tables"]["ficha_sensible_voluntario"]["Update"];
...
export async function saveBeneficiaryMedicalRecord(options: {
  ...
  const payload: BeneficiaryMedicalUpdatePayload = {
    tenant_id: tenantId,
  ...
  if (existing) {
    ...
    const { error } = await clinicoSchema()
      ...
      .insert({
        ...payload,
        created_by: actorId,
      } satisfies BeneficiaryMedicalInsertPayload);
...
export async function saveVolunteerSensitiveRecord(options: {
  ...
  const payload: VolunteerSensitiveUpdatePayload = {
    tenant_id: tenantId,
  ...
  if (data?.id) {
    ...
    const { error: insertError } = await clinicoSchema()
      ...
      .insert({
        ...payload,
        created_by: actorId,
      } satisfies VolunteerSensitiveInsertPayload);
