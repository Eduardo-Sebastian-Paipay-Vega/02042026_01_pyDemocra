...
const APPROVALS_SCOPE_WARNING =
  "La bandeja ya lee `ong.aprobaciones` como fuente principal y usa `ong.horas_actividad` solo como contexto vinculado, segun `guidelines/BD/Parte 4- Script maestro documental de ONG modulos complementarios.txt`.";
...
async function listApprovalRows(
  ...
  let query = ongSchema()
    ...
    .eq("entidad_tabla", HOURS_APPROVAL_ENTITY)
  ...
  if (stateFilter) {
    query = query.eq("estado", stateFilter as "pendiente" | "aprobada" | "rechazada" | "devuelta");
  }
  if (approvedById) {
...
function mapApprovalRow(
  ...
  return {
    ...
    entityTitle: linkedHours?.activityName ?? "Registro de horas",
...
export async function listAprobaciones(
  ...
  try {
    ...
    const entityTypeRaw = sanitizeText(filters.entityType ?? null, 80);
    const entityType = entityTypeRaw && entityTypeRaw !== "all" ? entityTypeRaw : null;

    if (entityType && entityType !== HOURS_APPROVAL_ENTITY) {
      warnings.push(
        `La vista actual solo expone aprobaciones de ${HOURS_APPROVAL_ENTITY} sobre \`ong.aprobaciones\`.`
      );
...
export async function createAprobacion(
  ...
  try {
    ...
    await requestHoursApproval({
      hoursId: target.id,
      requesterId: input.requestedBy ?? undefined,
...
export async function resolveAprobacion(
  ...
  try {
    ...
    if (
      ...
      throw new Error("La integracion actual solo resuelve aprobaciones de horas.");
    }
    ...
    const refreshedApproval =
      ...
      (await getLatestApprovalForEntity(tenantId, currentApproval.entidad_id));
    if (!refreshedApproval) {
      throw new Error("No se pudo recargar la aprobacion luego de resolverla.");
    }

    const linkedHours = await getHorasById(currentApproval.entidad_id).catch(() => null);
    const profileIds = [refreshedApproval.solicitado_por, refreshedApproval.resuelto_por].filter(
      (value): value is string => Boolean(value)
    );
    const profileLabels = await resolveProfileLabels(profileIds).catch(
