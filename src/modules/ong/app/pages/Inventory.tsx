...
const itemColumns: Column<any>[] = [
  { key: "item", label: "Item", render: (item) => <div><div style={{ color: "var(--t-text)" }}>{item.name}</div><div className="mt-0.5 text-[11px]" style={{ color: "var(--t-text-dim)" }}>{item.code}</div></div> },
...
const locationColumns: Column<any>[] = [
  { key: "name", label: "Ubicacion", render: (item) => <div><div style={{ color: "var(--t-text)" }}>{item.name}</div><div className="mt-0.5 text-[11px]" style={{ color: "var(--t-text-dim)" }}>{item.code}</div></div> },
...
export function Inventory() {
  ...
  const [itemForm, setItemForm] = useState({ code: "", name: "", description: "", unitCode: "", stateCode: "", active: true });
  ...
  const [locationForm, setLocationForm] = useState({ code: "", name: "", address: "", latitude: "", longitude: "", countryCode: "PE", active: true });
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      {view === "items" && (
        <>
          ...
          <DataTable columns={itemColumns} data={items.rows} loading={items.loading} emptyMessage="No se encontraron items." actions={[
            ...
            { label: "Editar", onClick: (row) => { setEditingItem(row); setItemForm({ code: row.code, name: row.name, description: row.description, unitCode: row.unitCode, stateCode: row.stateCode, active: row.active }); setItemError(null); setItemFormOpen(true); } },
          ...
          <GradientButton size="sm" onClick={() => { setEditingItem(null); setItemForm({ code: "", name: "", description: "", unitCode: items.unitOptions[0]?.value ?? "", stateCode: items.stateOptions[0]?.value ?? "", active: true }); setItemError(null); setItemFormOpen(true); }}>Nuevo item</GradientButton>
        </>
      )}
      ...
      {view === "locations" && (
        <>
          ...
          <GradientButton size="sm" onClick={() => { setEditingLocation(null); setLocationForm({ code: "", name: "", address: "", latitude: "", longitude: "", countryCode: locations.countryOptions[0]?.value ?? "PE", active: true }); setLocationError(null); setLocationFormOpen(true); }}>Nueva ubicacion</GradientButton>
        </>
      )}
      ...
      <ModalShell open={itemFormOpen} onClose={() => setItemFormOpen(false)} width="max-w-[860px]">
        ...
        <div className="space-y-3 p-4">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <input value={itemForm.code} onChange={(event) => setItemForm((current) => ({ ...current, code: event.target.value }))} placeholder="Codigo" className="h-9 rounded-xl px-3 text-[12px] outline-none" style={{ border: "1px solid var(--t-border)", background: "var(--t-input-bg)", color: "var(--t-text-secondary)" }} />
          ...
          <div className="flex gap-2">
            <GradientButton size="sm" onClick={async () => {
              if (!itemForm.code.trim() || !itemForm.name.trim() || !itemForm.unitCode || !itemForm.stateCode) { setItemError("Codigo, nombre, unidad y estado son obligatorios."); return; }
              try {
                const payload = { code: itemForm.code.trim(), name: itemForm.name.trim(), description: itemForm.description || null, unitCode: itemForm.unitCode, stateCode: itemForm.stateCode, active: itemForm.active };
        ...
        <div className="space-y-3 p-4">
          ...
          <div className="flex gap-2">
            <GradientButton size="sm" onClick={async () => {
              ...
              try {
                const payload = { code: locationForm.code.trim(), name: locationForm.name.trim(), address: locationForm.address || null, countryCode: locationForm.countryCode || null, latitude: locationForm.latitude ? Number(locationForm.latitude) : null, longitude: locationForm.longitude ? Number(locationForm.longitude) : null, active: locationForm.active };
