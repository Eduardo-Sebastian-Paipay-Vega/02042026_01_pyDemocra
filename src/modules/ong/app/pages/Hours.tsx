...
export function Hours() {
  ...
  function validateForm(mode: FormMode): boolean {
    ...
    if (formMinutes && parsedMinutes !== null && (Number.isNaN(parsedMinutes) || parsedMinutes <= 0)) {
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-6">
      <motion.div variants={fadeUp}>
        <PageHeader
          title="Horas"
          description="Registra y gestiona las horas de voluntarios por actividad. Desde aquí puedes solicitar aprobaciones y ver el estado de cada registro."
      ...
      <motion.div variants={fadeUp}>
        <div className="flex flex-wrap gap-2">
          <SelectField value={status} onChange={(v) => setStatus(v as typeof status)} options={statusOptions} />
          <SelectField
        ...
        >
          <div>
            ...
            <p className="text-[12px]" style={{ color: "var(--t-text-dim)" }}>
              Completa los campos del registro de horas para esta actividad.
            </p>
          </div>
          <button
            type="button"
