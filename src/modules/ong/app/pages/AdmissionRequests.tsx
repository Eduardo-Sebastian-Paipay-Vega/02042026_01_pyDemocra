...
export function AdmissionRequests() {
  ...
  const [searchTerm, setSearchTerm] = useSessionStorageState(
    "ong.view.admission.search",
    ""
  );
  const [status, setStatus] = useSessionStorageState<"all" | AdmissionStateKind>(
    "ong.view.admission.status",
    "all"
  );
  const [page, setPage] = useSessionStorageState("ong.view.admission.page", 1);
  const [detailRequestId, setDetailRequestId] = useState<string | null>(null);
  const [editingRequest, setEditingRequest] = useState<AdmissionRequestRow | null>(null);
  const [editingRequestId, setEditingRequestId] = useSessionStorageState<string | null>(
    "ong.view.admission.request-form.editing-id",
    null
  );
  ...
  const [isRequestFormOpen, setIsRequestFormOpen] = useSessionStorageState(
    "ong.view.admission.request-form.open",
    false
  );
  ...
  const isEditingRequest = Boolean(editingRequest?.id ?? editingRequestId);
  ...
  async function submitRequest() {
    ...
    const activeEditingRequestId = editingRequest?.id ?? editingRequestId;

    try {
      if (activeEditingRequestId) {
        const payload: AdmissionUpdateInput = { requestId: activeEditingRequestId, nombres: requestForm.nombres.trim(), apellidos: requestForm.apellidos.trim(), email: requestForm.email.trim(), notes: requestForm.notes.trim() || null };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <ModalShell open={isRequestFormOpen} onClose={() => setIsRequestFormOpen(false)} width="max-w-[760px]">
        <div className="flex items-start justify-between px-4 py-3" style={{ borderBottom: "1px solid var(--t-border)" }}>
          <div>
            <h3 className="text-[14px]" style={{ color: "var(--t-text)" }}>{isEditingRequest ? "Editar solicitud" : "Nueva solicitud"}</h3>
            <p className="text-[12px]" style={{ color: "var(--t-text-dim)" }}>{isEditingRequest ? "Actualiza el expediente." : "Registro real en rrhh.solicitudes_admision."}</p>
          </div>
        ...
        <div className="space-y-3 p-4">
          ...
          {!isEditingRequest && <SelectField value={requestForm.stateCode} onChange={(value) => setRequestForm((current) => ({ ...current, stateCode: value as AdmissionStateCode }))} options={stateOptions} disabled={admission.isCreating} />}
