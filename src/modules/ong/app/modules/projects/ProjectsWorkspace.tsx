...
export function ProjectsWorkspace({ section }: { section: ProjectModuleSection }) {
  ...
  async function handleSubmitForm() {
    try {
      if (section === "projects") {
        if (editingProjectId) {
          await mutations.updateProject(editingProjectId, projectForm);
          toast.success("Proyecto actualizado.");
        } else {
          await mutations.createProject(projectForm);
      ...
      render: (row) => (
        <div>
          <div className="flex items-center gap-2">
            <FolderKanban className="h-3.5 w-3.5" style={{ color: "var(--t-text-dim)" }} />
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {(Object.keys(SECTION_META) as ProjectModuleSection[]).map((item) => (
          <Link
            ...
            style={{
              borderColor: item === section ? "rgba(117,69,226,0.4)" : "var(--t-border)",
              background: item === section ? "rgba(117,69,226,0.12)" : "var(--t-input-bg)",
      ...
      <ModalShell open={formOpen} onClose={closeForm} width="max-w-[980px]">
        ...
        <div className="max-h-[75vh] space-y-4 overflow-y-auto p-4">
          {section === "projects" ? <div className="grid gap-3 md:grid-cols-2"><InputField value={projectForm.code} onChange={(value) => setProjectForm((current) => ({ ...current, code: value }))} placeholder="Codigo del proyecto" /><InputField value={projectForm.name} onChange={(value) => setProjectForm((current) => ({ ...current, name: value }))} placeholder="Nombre del proyecto" /><SelectField value={projectForm.areaId} onChange={(value) => setProjectForm((current) => ({ ...current, areaId: value }))} options={catalogs.areas} placeholder="Area" /><SelectField value={projectForm.stateCode} onChange={(value) => setProjectForm((current) => ({ ...current, stateCode: value }))} options={catalogs.projectStates} placeholder="Estado" /><InputField value={projectForm.startDate} onChange={(value) => setProjectForm((current) => ({ ...current, startDate: value }))} placeholder="Fecha inicio" type="date" /><InputField value={projectForm.endDate} onChange={(value) => setProjectForm((current) => ({ ...current, endDate: value }))} placeholder="Fecha fin" type="date" /><InputField value={projectForm.budget} onChange={(value) => setProjectForm((current) => ({ ...current, budget: value }))} placeholder="Presupuesto" type="number" /><InputField value={projectForm.imageUrl} onChange={(value) => setProjectForm((current) => ({ ...current, imageUrl: value }))} placeholder="Imagen URL (opcional)" /><div className="md:col-span-2"><TextareaField value={projectForm.description} onChange={(value) => setProjectForm((current) => ({ ...current, description: value }))} placeholder="Descripcion del proyecto" /></div></div> : null}
