...
-- Roles y permisos: lectura para tenant, escritura solo admins/permiso
drop policy if exists p_roles_select on public.roles;
create policy p_roles_select on public.roles
for select
using (tenant_id = public.fn_current_tenant_id());
...
-- user_roles_sedes: lectura del tenant y admin para cambios
drop policy if exists p_urs_select on public.user_roles_sedes;
create policy p_urs_select on public.user_roles_sedes
for select
using (tenant_id = public.fn_current_tenant_id());
...
create policy p_urs_write on public.user_roles_sedes
for insert, update, delete
using (tenant_id = public.fn_current_tenant_id() and (public.fn_has_permission('iam.user_roles.manage', null) or public.fn_is_tenant_admin()))
with check (tenant_id = public.fn_current_tenant_id());
...
-- Bloqueamos insert/update/delete directos a audit_logs (solo por trigger)
drop policy if exists p_audit_no_write on public.audit_logs;
create policy p_audit_no_write on public.audit_logs
for insert, update, delete
using (false)
with check (false);
...
create trigger tr_audit_urs
after insert or update or delete on public.user_roles_sedes
for each row execute function public.fn_trigger_audit_universal('user_id'); -- ojo: aquí no sirve tenant_id, ver nota abajo
-- NOTA: user_roles_sedes no tiene tenant_id directo. Para auditarlo perfecto se suele duplicar tenant_id.
-- Si quieres “perfecto”, añade tenant_id en user_roles_sedes. Abajo dejo el fix recomendado.
...
-- user_roles_sedes: lectura del tenant y admin para cambios (recomendado mejorar con tenant_id en tabla)
drop policy if exists p_urs_select on public.user_roles_sedes;
create policy p_urs_select on public.user_roles_sedes
for select
using (
  exists (
    select 1
    from public.roles r
    join public.sedes s on s.id = user_roles_sedes.sede_id
    where r.id = user_roles_sedes.role_id
      and r.tenant_id = public.fn_current_tenant_id()
      and s.tenant_id = public.fn_current_tenant_id()
  )
);
...
create policy p_urs_write on public.user_roles_sedes
for insert, update, delete
using (public.fn_has_permission('iam.user_roles.manage', null) or public.fn_is_tenant_admin())
with check (true);
...
-- ---------------------------------------------------------------------------
-- 12) FIX RECOMENDADO (PERFECTO): agregar tenant_id a user_roles_sedes
--     para auditoría y RLS sin joins raros
-- ---------------------------------------------------------------------------

alter table public.user_roles_sedes
add column if not exists tenant_id uuid;

-- Backfill rápido (si ya existiera data)
...
-- Re-crear auditoría correcta para user_roles_sedes ahora que existe tenant_id
drop trigger if exists tr_audit_urs on public.user_roles_sedes;
create trigger tr_audit_urs
after insert or update or delete on public.user_roles_sedes
for each row execute function public.fn_trigger_audit_universal('tenant_id');

-- Re-crear RLS policies más simple
drop policy if exists p_urs_select on public.user_roles_sedes;
create policy p_urs_select on public.user_roles_sedes
for select
using (tenant_id = public.fn_current_tenant_id());
...
-- =============================================================================
-- SUPABASE BOOTSTRAP: Permisos + función para crear tenant completo
-- (Pegar DESPUÉS del script principal)
-- =============================================================================
...
-- ---------------------------------------------------------------------------
-- E) RLS: permitir bootstrap controlado
-- Nota: security definer ejecuta como dueño, pero igual es buena práctica
-- ---------------------------------------------------------------------------

-- Permitir que usuarios autenticados creen su propio profile si falta
drop policy if exists p_profiles_insert on public.profiles;
create policy p_profiles_insert on public.profiles
for insert
with check (id = auth.uid());

-- Permitir update de su propio profile (para que se le setee tenant_id durante bootstrap)
drop policy if exists p_profiles_update on public.profiles;
create policy p_profiles_update on public.profiles
for update
using (
...
-- IMPORTANTE:
-- Si quieres que SOLO se pueda crear tenant por esta función (y no insert directo),
-- puedes deshabilitar inserts directos en tenants con policy (opcional) y manejarlo solo por RPC.
